...
스플라이스는 기존의 아레이를 mutate한다.
중간의 element를 하나이상으로 없앨수 있다.
splice(자르기 시작하고 싶은 인덱스, 얼마나 많은 엘리먼트를 없앨지) 6. Add Items Using splice()
...
- slice() 이렇게만하면 그냥 아레이 카피본을 만들어 버림, 기존의 array를 mutate하지 않는다.
